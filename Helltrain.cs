...
private void EnsureRespawnScheduledFromStateOrDefault()
{
    ...
    // FixedSchedule: всегда пересчитываем строго по слотам, state не используем как источник времени старта
...
private void StartRespawnTimer(float? overrideMinutes = null)
{
    ...
    if (useFixedSchedule)
    {
        ...
        {
            PrintWarning("[Helltrain] FixedSchedule enabled, but no valid windows found. Respawn timer is not scheduled.");
            _nextRespawnUtc = null;
            SaveRespawnState();
            return;
        }
